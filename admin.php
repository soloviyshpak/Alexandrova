...
if(isset($_SESSION['isAdmin']) && $_SESSION['isAdmin'] == 'true') {
    ...
    $sql = "SELECT users.name, users.gender, users.email, orders.type, orders.userId
            ...
            WHERE orders.state = 'На рассмотрении'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while($row = mysqli_fetch_assoc($result)) {
            echo '
              <li class="admin-orders__item">
                ...
                <p class="admin-orders__item-info">'.$row["name"].' ('.$row["gender"].')</p>
        ...
        echo "Нет заказов на рассмотрении.";
    }
        echo '
          </ul>
            <a class="admin-orders__accepted" href="admin-accepted.php">смотреть принятые заказы</a>
          </section>
        </main>
      </body>
